...
app.get('/live/:hashed', jsonParser, urlEncodeHandler, function(req, res) {
    ...
    if (decryptedMessage) {
        ...
        var xhost = 'http://127.0.0.1:' + process.argv[3] + '/output';
        var xsocket = null;
...
app.post('/v1/playStream/:streamID', jsonParser, urlEncodeHandler, sessionAuth, function(req, res) {
    ...
    function startStream(relationship) {
        return res.json(outputResult(extend({} , {'auth' : hashAuth}, config.app.stream)));
    }
});
...
function getUserDir(userID) {
    return config.directory.user_content + '/' + encodeStr(userID) + '/';
}

function getStreamDir(streamID) {
    return config.directory.stream_content + '/' + encodeStr(streamID) + '/';
}
