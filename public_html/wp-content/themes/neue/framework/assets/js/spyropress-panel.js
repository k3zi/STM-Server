...
;(function($) {
	...
	panelUi = {
		...
		init_select_wrapper: function() {
			...
			// Enable Changer
			var select_changer = $( '.enable_changer select, .enable_changer input:radio, #page_template');
		...
		gallery: {
            ...
            init: function() {
                ...
                $('body').on( 'click', '.gallery_upload_button', function( e ) {
    				e.preventDefault();
                    ...
                    wp.media.gallery.shortcode = function( attachments ) {
						...
						// generate html
						g.gallery_generator( attachments, $gallery );

						// restore original
						wp.media.gallery.shortcode = clone;
						var sc = clone( attachments );
						$shortcode.val( sc.string() );
						return sc;
					};
			...
			gallery_generator: function(attachments, $gallery) {
				...
				attachments.map(function(attachment) {
					...
					image = $('<div data-id="' + attachment.id + '" class="gallery-item" style="display:none">' + '<img src="' + img + '" /></div>').hide();
