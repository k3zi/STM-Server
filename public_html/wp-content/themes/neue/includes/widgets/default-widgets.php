...
<?php
		...
		$out = wp_list_categories(apply_filters('widget_categories_args', $cat_args));
         $out = str_replace( '<li', '<li class="cat-item"' , $out );
        //$out = str_replace( '<span>', '<b class="count">', $out );
        //$out = str_replace( '</span>', '</b>', $out );
        echo $out;
?>
		</ul>
<?php
...
?>
		<ul class="nav nav-list primary pull-bottom">
		      <?php 
                    $archives = wp_get_archives(apply_filters('widget_archives_args', array('type' => 'monthly', 'show_post_count' => $c , 'echo' => 0))); 
                    echo str_replace('<li>' , '<li class="cat-item">' , $archives);
              ?>
		</ul>
<?php
		}
...
?>
			...
			<li class="cat-item"><a href="<?php bloginfo('rss2_url'); ?>" title="<?php echo esc_attr(__('Syndicate this site using RSS 2.0')); ?>"><?php _e('Entries <abbr title="Really Simple Syndication">RSS</abbr>'); ?></a></li>
			<li class="cat-item"><a href="<?php bloginfo('comments_rss2_url'); ?>" title="<?php echo esc_attr(__('The latest comments to all posts in RSS')); ?>"><?php _e('Comments <abbr title="Really Simple Syndication">RSS</abbr>'); ?></a></li>
			<?php echo apply_filters( 'widget_meta_poweredby', sprintf( '<li class="cat-item"><a href="%s" title="%s">%s</a></li>',
...
class Spyropress_Widget_Custom_Nav extends WP_Nav_Menu_Widget {
    ...
    function widget($args, $instance) {
        ...
        echo str_replace( 'class="internal"' , 'class="cat-item"' , $nav );
		...
		if ( $comments ) {
			...
			foreach ( (array) $comments as $comment) {
                 ...
                 <li class="recentcomments">
                	<div class="post-info">
                        ' . sprintf(_x('%1$s on %2$s', 'widgets'), get_comment_author(), '<a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '">' . get_the_title($comment->comment_post_ID) . '</a>') . '
                	</div>
                </li>';
			}
 		}
...
class Spyropress_Widget_Tag extends WP_Widget_Tag_Cloud {
...
register_widget( 'Spyropress_Widget_Tag' );

class Spyropress_WP_Widget_Calendar extends WP_Widget_Calendar {
...
register_widget( 'Spyropress_WP_Widget_Calendar' );       
?>